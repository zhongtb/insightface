...
from mxnetDetnet.operator_py.pyramid_proposal import *
from mxnetDetnet.operator_py.proposal_target import *
from mxnetDetnet.operator_py.fpn_roi_pooling import *
...
def get_fixed_params(symbol, fixed_param):
    ...
    for name in symbol.list_arguments():
      ...
      if config.PLATFORM_DEVICES:
          if idx<9 and idx>2 and name!='data':
              fixed_param_names.append(name)
...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch,
    ...
    sym = None
    #sym = mx.symbol.load('model/detnet_coco-symbol.json')
    ...
    # train
    mod.fit(train_data, eval_metric=eval_metrics, epoch_end_callback=_epoch_callback,
