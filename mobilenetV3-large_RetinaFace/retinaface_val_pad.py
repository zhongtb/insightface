...
class RetinaFace:
  def __init__(self, prefix, epoch, ctx_id=1, network='net3a', dense_mode=False,nms=0.4, nocrop=False, decay4 = 0.5, vote=False):
    self.ctx_id = ctx_id
    ...
    if network=='ssh' or network=='vgg':
      ...
      fmc = 4
      _ratio = (1.,1.5)
    elif network=='detnet': #detnet 4 8 16
      fmc = 1
      _ratio = (1.,1.5)
    elif network=='detnet1': #detnet 4 8 16
      fmc = 0
      _ratio = (1.)
    else:
    ...
    #dense_anchor =False# True#False
    #dense_anchor = dense_mode
    dense_anchor = False
    print('dense_anchor: ', dense_anchor)
    print('_cfg_anchors: ', self.anchor_cfg)
  ...
  def detect(self, img, threshold=0.5, scales=[1.0],target_scales=[720,1280], do_flip=False):
  #def detect(self, img, threshold=0.5, scales=[1.0],target_scales=[320,320], do_flip=False):
    ...
    for im_scale in scales:
      for flip in flips:
        if im_scale!=1.0:
          ...
          im = img.copy()
        im_height = int(np.ceil(im.shape[1]))
        im_width = int(np.ceil(im.shape[0]))
        im_shape = im.shape
        #print('imgg_shape: ', img.shape)
        #print('imggggg_shape: ', im.shape)
        im_channel = im.shape[2]
        padded_im = np.zeros((target_scales[0], target_scales[1], im_channel))
        padded_im[:im.shape[0], :im.shape[1], :] = im
        im = padded_im
        #cv2.imwrite('pad.jpg', im)
        ...
        for _idx,s in enumerate(self._feat_stride_fpn):
            ...
            im_scale_x = im_shape[0]/img.shape[0]
            #print('im_scale_x: ',im_scale_x)
            im_scale_y = im_shape[1]/img.shape[1]
            #print('im_scale_y: ',im_scale_y)
            proposals[:,0:1] /= im_scale_x
            proposals[:,1:2] /= im_scale_y
            proposals[:,2:3] /= im_scale_x
            proposals[:,3:4] /= im_scale_y
  ...
  def bbox_pred(boxes, box_deltas):
      ...
      #print(pred_boxes.shape, pred_ctr_x.shape, pred_w.shape)
      # x1
