...
class RetinaFace:
  def __init__(self, prefix, epoch, ctx_id=1, network='net3a', dense_anchor=False,nms=0.4, nocrop=False, decay4 = 0.5, vote=False):
    self.ctx_id = ctx_id
    ...
    self.model.bind(data_shapes=[('data', (1, 1, image_size[0], image_size[1]))], for_training=False)
  ...
  def get_input(self, img):
    ...
    for i in range(1):
        im_tensor[0, i, :, :] = (im[:, :, i]/self.pixel_scale - self.pixel_means[i])/self.pixel_stds[i]
    #if self.debug:
